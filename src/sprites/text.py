...
class Text(py.sprite.Sprite):
    def __init__(self, text, position=(0, 0), on_click=None, font_size=12, is_centered=False, layer=0, color=(255,255,255)):
        self._layer = layer
        self.font = py.font.SysFont("Segoe UI", font_size)
        py.sprite.Sprite.__init__(self, (groups.all, groups.layeredItems, groups.overlays))
        self.image, self.rect = text_objects(text, self.font, position, is_centered, color)
